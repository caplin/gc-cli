...
import { addAliasesRequiresVisitor, orMatchers, literalMatcher, composeMatchers, identifierMatcher, callExpressionMatcher, memberExpressionMatcher, variableDeclaratorMatcher, extractParent, extractProperties, composeTransformers, moduleIdVisitor, rootNamespaceVisitor, nodePathLocatorVisitor, flattenMemberExpression, cjsRequireRemoverVisitor, verifyVarIsAvailableVisitor, varNamespaceAliasExpanderVisitor, addRequireForGlobalIdentifierVisitor, removeRedundantRequiresVisitor, replaceLibraryIncludesWithRequiresVisitor } from '../../global-compiler';

import { getFileNamespaceParts, transformASTAndPushToNextStream } from './utils/utilities';

import { getServiceMatchers } from './matchers/service-registry';

import { getServiceNodesReceiver } from './receivers/service-registry';

const { builders: { literal, identifier } } = types;

const caplinRequireMatcher = composeMatchers(literalMatcher('caplin'), callExpressionMatcher({ callee: identifierMatcher('require') }), variableDeclaratorMatcher({ id: identifierMatcher('caplin') }));

const caplinInheritanceMatcher = composeMatchers(identifierMatcher('caplin'), orMatchers(memberExpressionMatcher({ property: identifierMatcher('extend') }), memberExpressionMatcher({ property: identifierMatcher('implement') })), callExpressionMatcher());
...
const caplinRequireTransformer = composeTransformers(literal('topiarist'), extractParent(), extractParent(), extractProperties('id'), identifier('topiarist'));

const caplinInheritanceToExtendTransformer = composeTransformers(identifier('topiarist'), extractParent(), extractProperties('property'), identifier('extend'));

const caplinInheritanceToInheritTransformer = composeTransformers(identifier('topiarist'), extractParent(), extractProperties('property'), identifier('inherit'));
...
export function transformSLJSUsage() {
	return through2.obj(function (fileMetadata, encoding, callback) {
		...
		const libraryIdentifiersToRequire = new Map([[Iterable([freeSLJSVariation]), 'sljs']]);
...
export function addRequiresForCaplinBootstrap() {
	const caplinBootstrapIdentifier = new Map([[List.of('caplin'), 'caplinBootstrap']]);
...
export function transformI18nUsage() {
	return through2.obj(function (fileMetadata, encoding, callback) {
		...
		const moduleIdsToConvert = new Map([['ct', ['br/I18n', freeI18NVariation]], ['br', ['br/I18n', freeI18NVariation]]]);
